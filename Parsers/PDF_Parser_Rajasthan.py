...
case_regex = re.compile(r'((CRP\(C.R.P.)|RES.APPL.|I.T. APPEAL|CRL.M.\(BAIL\)|((of|OF|\/|-)(\s)?[0-9]{4})(\s)?(\([-a-zA-Z0-9\s]+\)$)?)')
            ...
            if (values < len(Case_Numbers) -1):
                ...
                for index in range(len(data)):
                    if (index >= Index_1 and index < Index_2):
                        ...
                        if (date_regex_acceptance.search(data[index]['Line_Data']['Value'])):
                            ...
                            pass
                    if (index > Index_1 and index < Index_2) and "VKJNGT+CourierNew,Bold" not in data[index]['Line_Data']['font_name'] and len(data[index]['Line_Data']['Value']) < 70 and not(date_regex.search(data[index]['Line_Data']['Value'])) and data[index]['Line_Data']['leftpoint_x'] > Case_Numbers[values]['Left_Point'] and data[index]['Line_Data']['leftpoint_x'] > 100 and "HON'BLE" not in data[index]['Line_Data']['Value'] and "file" not in data[index]['Line_Data']['Value'] and "admission" not in data[index]['Line_Data']['Value'].lower() and "hearing" not in data[index]['Line_Data']['Value'].lower() and "order" not in data[index]['Line_Data']['Value'].lower() and "part" not in data[index]['Line_Data']['Value'].lower() and " pm" not in data[index]['Line_Data']['Value'].lower() and "bench" not in data[index]['Line_Data']['Value'].lower() and "name of advocate" not in data[index]['Line_Data']['Value'].lower():
                ...
                for ind in range(len(Batch)):
                    if (ind < len(Batch) -1):
                        ...
                        Party_Name.append(Batch[ind]['Line_Data']['Value'].replace("\xa0",""))
                    elif (ind == len(Batch) - 1) and 350 > Batch[ind]['Line_Data']['leftpoint_x'] > 290:
                        Judge_Name.append(Batch[ind]['Line_Data']['Value'].replace("\xa0",""))
                ...
                for name in Cleaned_Judge.replace("\n", "").split("-"):
                    if (name == "P" or name.strip() == ""):
                        pass
                    else:
                        advocate_name = {"name": name.replace(" P", "").replace("P ", "")}
                        advocate_names_all_petitioner.append(advocate_name)
                JSON_Data["petitioner_advocate_name"] = advocate_names_all_petitioner
                for name in Cleaned_Adv_Name.replace("\n", "").split("-"):
                    if (name == "R" or name.strip() == ""):
                        pass
                    else:
                        advocate_name = {"name": name.replace(" R", "").replace("R ", "")}
                        advocate_names_all_respondent.append(advocate_name)
                JSON_Data["respondent_advocate_name"] = advocate_names_all_respondent
