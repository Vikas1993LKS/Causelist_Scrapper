...
listing_details = re.compile(r'(FOR ADMISSION|FOR ORDERS|FRESH ADMISSION)', re.IGNORECASE)
regexp = re.compile(r'^([0-9]{1,3}(\)|\.))')
case_regex = re.compile(r'(((of|OF|\/|-)(\s)?[0-9]{4})(\s)?(\([-a-zA-Z0-9\s]+\)$)?)')
case_regex_2nd = re.compile(r'^([0-9]{1,3}(\)|\.)).+?(((of|OF|\/|-)(\s)?[0-9]{4})(\s)?(\([-a-zA-Z0-9\s]+\)$)?)')
...
def parsepdf(data, download_dir):
        ...
        for values in range(len(Case_Numbers)):
            ...
            if (values < len(Case_Numbers) -1):
                ...
                for index in range(len(data)):
                    ...
                    if (index >= Index_1 and index < Index_2):
                        if (case_regex.search(data[index]['Line_Data']['Value']) or case_regex_2nd.search(data[index]['Line_Data']['Value'])) and not(date_regex.search(data[index]['Line_Data']['Value'])) and "file" not in data[index]['Line_Data']['Value'] and data[index]['Line_Data']['leftpoint_x'] < 160:
                            Case_regex_1 = case_regex_2nd.search(data[index]['Line_Data']['Value'])
                            if (Case_regex_1 != None): 
                                Replacement_Value = Case_regex_1.group(1)
                                Case_Numb.append(data[index]['Line_Data']['Value'].replace(Replacement_Value, "").strip())
                            else:
                ...
                Cleaned_Adv_Name = " ".join(Advocate_Names)
                if len(Case_Numb) != 0:
                    if (len(Party_Name) == 0):
                ...
                Advocate_Name.append(Cleaned_Adv_Name.replace("\n",", "))
                Batches.append(Batch)
                JSON_Data = {"case_numbers" : Cleaned_Case_Numbers} 
                JSON_Data["party_name"] = Cleaned_Party.replace("\n","").strip()
                JSON_Data["advocate_names"] = Cleaned_Adv_Name.strip().replace("\n",",")[:-1]
                JSON_Data["state"] = "Odisha"
                ...
                for value in range(len(data)):
                    if value >= Index_Number['Index'] and value <= (Index_Number['Index'] + 10):
                        if data[value]['Line_Data']['leftpoint_x'] < 130 and (case_regex_2nd.search(data[value]['Line_Data']['Value']) or (case_regex.search(data[value]['Line_Data']['Value']))):
                            Case_Numb.append(re.sub(r'([0-9]{1,3}(\)|\.))',"" ,data[value]['Line_Data']['Value'].strip()))
                        elif (130 < data[value]['Line_Data']['leftpoint_x'] <= 280 and not(date_regex.search(data[value]['Line_Data']['Value']))):
                            Party_Name.append(data[value]['Line_Data']['Value'].strip())
                        elif (280 < data[value]['Line_Data']['leftpoint_x'] < 400 and not(date_regex.search(data[value]['Line_Data']['Value']))):
                            Advocate_Names.append(data[value]['Line_Data']['Value'].strip())
                        else:
                            pass
                Cleaned_Case_Numbers = ", ".join(Case_Numb)
                Cleaned_Adv_Name = " ".join(Advocate_Names)
...
#         df1 = pd.DataFrame({'Case_Number':Case_Num})
#         df2 = pd.DataFrame({'Party_Names':Party})
#         df3 = pd.DataFrame({'Advocate_Names': Advocate_Name})
#         df4 = pd.DataFrame({'Listing_Details': Hearing_Types})
