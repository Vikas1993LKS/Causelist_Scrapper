...
def parsepdf(data, download_dir):
    ...
    #         print (file_name)
        ...
        for values in range(len(Case_Numbers)):
            ...
            if (values < len(Case_Numbers) -1):
                ...
                for index in range(len(data)):
                    if (data[index]['Line_Data']['font_size'] > 16 and "bold" in data[index]['Line_Data']['font_name'].lower() and "vs." not in data[index]['Line_Data']['Value'].lower() and "with" not in data[index]['Line_Data']['Value'].lower()):
                ...
                JSON_Data['petitioner_advocate_names'] = Cleaned_Petiotioner_Adv
                Cleaned_Party = " ".join(Party_Name)
                JSON_Data['party_names'] = Cleaned_Party
                Cleaned_Respondent_Adv = " ".join(Respondent_Advocates)
                JSON_Data['respondent_advocate_names'] = Cleaned_Respondent_Adv
                JSON_Data['state'] = "Meghalya"
                if len(Case_Numb) != 0:
                    if (len(Party_Name) == 0):
        ...
        # df3 = pd.DataFrame({'Appellant Advocate Names': Petitioner_Advocate})
